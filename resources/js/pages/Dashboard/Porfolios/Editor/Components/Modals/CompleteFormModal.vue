    ...
    <Transition name="modal-fade">
        <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" @click="$emit('close')">
            ...
            <Transition name="modal-scale" mode="out-in">
                <div v-if="isRedirecting" key="success" class="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl" @click.stop>
                    
                    <!-- HEADER - ÉXITO -->
                    <div class="border-b border-green-100 bg-gradient-to-r from-green-50 to-emerald-50 px-6 py-8 text-center">
                    ...
                    <div class="p-6">
                        ...
                        <div class="mb-6 rounded-lg bg-amber-50 p-4 border border-amber-100">
                            <p class="text-center text-sm text-amber-900">
                                <span class="font-semibold">Redirigiendo en {{ redirectCountdown }}s...</span>
                            </p>
                            <div class="mt-3 h-1 w-full overflow-hidden rounded-full bg-amber-200">
                                <div 
                        ...
                        <button 
                            ...
                            class="w-full rounded-lg bg-[#005aeb] px-4 py-2.5 font-medium text-white transition-colors hover:bg-[#0048c4] flex items-center justify-center gap-2"
                        >
                ...
                <!-- ============================================ -->
                <div v-else key="form" class="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-xl" @click.stop>
                        ...
                        <div v-else>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <button @click="handleBackToEditor()" @click.stop="$emit('close')" :disabled="isCompleting"
                                    class="flex items-center justify-center space-x-2 rounded-lg border border-gray-300 px-4 py-2.5 text-gray-700 font-medium hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                                    <ArrowLeft class="h-4 w-4" />
                                    <span>Volver</span>
                                </button>

                                <button @click="handleCompleteClick()" @click.stop="$emit('complete')" :disabled="isCompleting"
                                    class="flex items-center justify-center space-x-2 rounded-lg bg-green-500 px-4 py-2.5 text-white font-medium hover:bg-green-600 disabled:opacity-70 disabled:cursor-not-allowed transition-all">
