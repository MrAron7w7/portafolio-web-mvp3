...
const getQualityBadge = (text: string | undefined) => {
    ...
    if (length < 30) return 'bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-slate-400 border-gray-200 dark:border-slate-700';
    if (length < 80) return 'bg-yellow-50 dark:bg-yellow-900/10 text-yellow-700 dark:text-yellow-400 border-yellow-200 dark:border-yellow-900/30';
    if (length < 150) return 'bg-blue-50 dark:bg-blue-900/10 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-900/30';
    return 'bg-emerald-50 dark:bg-emerald-900/10 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-900/30';
};
...
<template>
    <div class="relative" ref="containerRef">
        <div class="mb-8">
            <h1 class="mb-3 text-2xl font-bold text-gray-900 dark:text-white lg:text-3xl">
                Experiencia Laboral
            </h1>
        ...
        <div class="space-y-3">
            ...
            <div v-for="(exp, index) in modelValue" :key="exp.id"
                        ...
                        <div class="flex flex-col">
                            <div class="flex items-center gap-2">
                                <span class="text-base font-medium transition-colors duration-300" :class="{
                                    'text-red-700 dark:text-red-400': hasExperienceError(index),
                                    'text-emerald-700 dark:text-emerald-400': newItems.has(exp.id),
                                    'text-gray-900 dark:text-white': !hasExperienceError(index) && !newItems.has(exp.id)
                                }">
                ...
                <transition enter-active-class="transition-all duration-300 ease-out"
                    ...
                    <div v-if="expandedIndex === exp.id" class="border-t px-4 py-4 bg-white/50 dark:bg-slate-900/50"
                        :class="hasExperienceError(index) ? 'border-red-200 dark:border-red-500/30' : 'border-gray-200 dark:border-slate-800'">
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input :value="exp.endDate" @input="
                                    ...
                                    class="w-full rounded-lg border dark:border-slate-800 bg-gray-50 dark:bg-slate-800 px-3 py-2 text-base text-gray-900 dark:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-[#005aeb]/20 disabled:cursor-not-allowed disabled:opacity-50"
                            ...
                            <div class="md:col-span-2">
                                <label class="flex cursor-pointer items-center space-x-3">
                                    <input :checked="exp.current" @change="
                                        ...
                                        " type="checkbox"
                                        class="h-4 w-4 cursor-pointer rounded border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-[#005aeb] focus:ring-[#005aeb]" />
...
<div class="md:col-span-2">
    ...
    <div class="relative">
        <textarea 
            ...
            rows="8"
            class="w-full rounded-lg border dark:border-slate-800 bg-gray-50 dark:bg-slate-800 px-3 py-2 pr-10 text-base text-gray-900 dark:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-[#005aeb]/20"
        ...
        <button
            ...
            type="button"
            class="absolute bottom-3 right-3 p-2 rounded-lg bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/40 border border-blue-200 dark:border-blue-900/30 hover:border-blue-300 dark:hover:border-blue-800 transition-all text-blue-600 dark:text-blue-400"
    ...
    <div 
        ...
        class="mt-2 inline-flex items-center gap-1 px-2 py-1 rounded-full bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-900/30 text-xs text-emerald-700 dark:text-emerald-400"
    >
        ...
        <button
            ...
            type="button"
            class="ml-1 hover:text-emerald-900 dark:hover:text-emerald-200"
        >
            ✕
        </button>
    </div>
    ...
    >
        <div 
            ...
            class="mt-3 space-y-3 p-3 rounded-lg bg-gradient-to-b from-blue-50 to-blue-25 dark:from-slate-800 dark:to-slate-900 border border-blue-200 dark:border-slate-700 shadow-lg overflow-hidden"
        >
            <div class="flex items-center justify-between">
                <p class="text-xs font-semibold text-gray-700 dark:text-slate-300 flex items-center gap-1">
                    <Sparkles class="h-3.5 w-3.5 text-blue-500 dark:text-blue-400" />
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <button 
                    ...
                    type="button"
                    class="text-left px-3 py-2.5 rounded-lg bg-white dark:bg-slate-800 border border-blue-100 dark:border-slate-700 hover:border-blue-400 dark:hover:border-blue-500 hover:bg-blue-50 dark:hover:bg-slate-700 transition-all text-xs text-gray-700 dark:text-slate-300 font-medium flex flex-col gap-1 group"
                >
                    <span class="text-blue-600 dark:text-blue-400 group-hover:text-blue-700 dark:group-hover:text-blue-300">{{ suggestion.label }}</span>
                    <span class="text-gray-500 dark:text-slate-400 text-xs leading-snug line-clamp-2">{{ suggestion.text }}</span>
                </button>
            </div>
        ...
        <transition enter-active-class="transition-all duration-200 ease-out"
            ...
            <div v-if="deleteConfirmIndex !== null"
                ...
                <div v-if="popoverPosition.positionY === 'above'"
                    class="absolute -bottom-2 h-4 w-4 rotate-45 border-b border-r border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900"
                    :style="{ left: popoverPosition.arrowLeft + 'px' }"></div>

                <div class="relative">
                    <div class="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-red-100 dark:bg-red-900/20">
                    ...
                    <div class="flex gap-2">
                        <button @click.stop="closeDeleteConfirm"
                            class="flex-1 rounded-lg border border-gray-300 dark:border-slate-700 px-3 py-2 text-xs font-medium text-gray-700 dark:text-slate-300 transition-colors hover:bg-gray-50 dark:hover:bg-slate-800">
                            Cancelar
                        </button>
